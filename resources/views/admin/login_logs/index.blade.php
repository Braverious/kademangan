                        ...
                        <div class="row align-items-end">
                                ...
                                <div class="form-group p-0">
                                    ...
                                    <select name="device" class="form-control">
                                        ...
                                        <option value="Laptop/PC"
                                            {{ request('device') == 'Laptop/PC' ? 'selected' : '' }}>Laptop/PC</option>
                                        <option value="Smartphone"
                                            {{ request('device') == 'Smartphone' ? 'selected' : '' }}>Smartphone
                                        </option>
                                        <option value="Tablet" {{ request('device') == 'Tablet' ? 'selected' : '' }}>
                                            Tablet</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group p-0">
                                    ...
                                    <input type="text" name="q" class="form-control" placeholder="Ketik NIK..."
                                        value="{{ request('q') }}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group p-0 d-flex gap-2">
                                    <button type="submit" class="btn btn-primary"><i class="fa fa-search"></i>
                                        Filter</button>
                                    <a href="{{ route('admin.settings.login_logs') }}"
                                        class="btn btn-secondary border"><i class="fa fa-sync"></i></a>

                                    <!-- Tombol Ekspor (Menggunakan value yang ditangkap Controller) -->
                                    <button type="submit" name="export" value="csv" class="btn btn-success ml-auto"
                                        title="Export CSV"><i class="fa fa-file-csv"></i></button>
                                    <button type="submit" name="export" value="pdf" class="btn btn-danger"
                                        title="Export PDF"><i class="fa fa-file-pdf"></i></button>
                                </div>
                            </div>
                        </div>
                    </form>
                                ...
                                @forelse ($logs as $log)
                                    <tr>
                                        <td>
                                            ...
                                            <small class="text-muted">{{ $log->created_at->format('H:i:s') }}
                                                WIB</small>
                                        </td>
                                        <td>
                                            @if ($log->user)
                                        ...
                                        <td>
                                            ...
                                            <small class="text-muted"><i class="fa fa-map-marker-alt"></i>
                                                {{ $log->location ?? 'Unknown' }}</small>
                                        </td>
                                        <td>
                                            <i
                                                class="fa {{ $log->device == 'Smartphone' ? 'fa-mobile-alt' : 'fa-laptop' }} mr-1"></i>
                                            {{ $log->device }}<br>
                                            <small class="text-primary"><i class="fab fa-chrome mr-1"></i>
                                                {{ $log->browser }}</small>
                                        </td>
                                    </tr>
                                @empty
